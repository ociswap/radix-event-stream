...
pub struct AppState {
    pub number: u64,
    pub transaction: Arc<Mutex<Option<sqlx::Transaction<'static, Sqlite>>>>,
...
pub fn handle_instantiate_event(
    context: EventHandlerContext<AppState, TxCtx>,
    event: Vec<u8>,
    tx_ctx: Option<&mut TxCtx>,
) -> Pin<Box<dyn Future<Output = Result<(), EventHandlerError>> + Send + 'static>>
{
    Box::pin(async move {
